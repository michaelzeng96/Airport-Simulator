...
extern Random my_random; // Shares my_random global variable declared in Simulator.h
...
private:
	...
	std::queue<Plane *> queue;           // queue of planes in the service queue
public:
	ServiceQueue() {}
	...
	void update(int clock) 
	{
		...
		if (!queue.empty()) {
			...
			{  
				...
				queue.pop();
				departure_queue->queue.push(plane);
			}
		}
		...
		if (queue.empty()) {
			...
			if (!landing_queue->queue.empty()) {

				Plane *plane = landing_queue->queue.front();
				landing_queue->queue.pop();
